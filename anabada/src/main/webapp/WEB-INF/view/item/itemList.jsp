...
<div class="container-fluid page-header py-5">
	...
	<ol class="breadcrumb justify-content-center mb-0">
		<li class="breadcrumb-item active text-white">itemList.do</li>
	</ol>
</div>
				...
				<div class="row g-4">
					...
					<div class="col-lg-9">
						...
						<div class="center">
							...
							<c:if test="${page.prev }">
							  <a href="itemList.do?page=${page.startPage - 1 }&radio=${radio }&keyword=${keyword }"> &laquo; </a>
							</c:if>
							<c:forEach begin="${page.startPage }" end="${page.endPage }" var="p">
							  <c:choose>
							    <c:when test="${p eq page.page }">
								  <a href="itemList.do?page=${p }&searchCondition=${searchCondition }&keyword=${keyword }" class="active">${p }</a>
								</c:when>
								<c:otherwise>
								  <a href="itemList.do?page=${p }&searchCondition=${searchCondition }&keyword=${keyword }">${p }</a>
								</c:otherwise>
							  </c:choose>
							</c:forEach>
							<c:if test="${page.next }">
							  <a href="itemList.do?page=${page.endPage + 1 }&radio=${radio }&keyword=${keyword }"> &raquo; </a>
							</c:if>
							</div>
						</div>
						<!-- 페이지공간 end. -->
